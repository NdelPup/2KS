...
#logging.basicConfig(level=logging.DEBUG)  #attiva il debug di stomp
#connessione al broker
conn = stomp.Connection([('192.168.1.2', 61613)])
#istanza classe led
ledController = led()
#istanza classe led
buzzController = buzzer()
...
def syncronize():
    ...
    conn.send(body="SYNC", destination='/queue/server', headers = { "sender" : "0001"})
...
def stompConnection():
    ...
    conn.subscribe(destination='/queue/lettore0001', id=0, ack='auto')
...
def checkTicket(ticket):
    ...
    if tickArena == arena and tickEvent == event and tickGate == gate:
        conn.send(body=ticket, destination='/queue/server', headers = { "sender" : "0001", "gate" : gate})
    else:
        ledController.led_rosso_blink()
